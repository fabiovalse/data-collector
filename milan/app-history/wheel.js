(function () {
  ...
  function setup() {
    const contentRadius = OUTER_RADIUS + HUMIDITY_RING_WIDTH + HUMIDITY_GAP + 30;
  ...
  // Multi-stop color scale for outdoor Milan weather
  const tempColorScale = d3.scaleLinear()
    .domain([-5, 5, 15, 25, 35])
    .range(['#1565c0', '#4fc3f7', '#81c784', '#ffb74d', '#e53935'])
  ...
  function drawWheel(days, label) {
    ...
    const nicestDay = days.reduce((best, d) => {
      // Score: close to 23°C mean + small range = nice stable warm day
    ...
    // Title
    const titleText = label === 'all' ? 'MILAN' : label;
    g.append('text')
      .attr('class', 'title')
      .attr('dy', '-0.2em')
      .text(titleText);
    ...
    g.append('text')
      .attr('class', 'subtitle')
      .attr('dy', '1.2em')
      .text(label === 'all' ? '2021 – 2026' : `${days.length} days`);
  }
